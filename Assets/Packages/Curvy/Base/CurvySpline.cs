...
namespace FluffyUnderware.Curvy
{
    ...
    public class CurvySpline : CurvySplineBase
    {
        ...
        public override Vector3 MoveByLengthFast(ref float tf, ref int direction, float distance, CurvyClamping clamping)
        {
            ...
            {
                // Convert unit distance to tf distance
                float dist = TFToDistance(tf);
                float tfDistance=0;
                if (direction >= 0)
                {
                    float newDist = dist + distance;
                    tfDistance = DistanceToTF(newDist % Length) - tf + Mathf.FloorToInt(newDist / Length);
                }
                else
                {
                    float newDist = dist - (distance % Length);
                    if (newDist < 0)
                    {
                        newDist += Length;
                        tfDistance = Mathf.CeilToInt(newDist / Length);
                    }
                    tfDistance += tf - DistanceToTF(newDist);
                }
                return eventAwareMove(ref tf, ref direction, tfDistance, clamping, true);
            }
        }
        ...
        CurvySplineSegment getNextSegmentWithinRange(CurvySplineSegment seg, float tf, float fDistance, out float segTF)
        {
            ...
            if (seg)
            {
                ...
                else if (fDistance < 0 && tf > 0)
                {
                    ...
                    segTF = SegmentToTF(next);
                    return (Mathf.Abs(tf - segTF) <= (fDistance * -1)) ? next : null;
                }
            }
            return null;
        }
        ...
        Vector3 eventAwareMove(ref float tf, ref int direction, float fDistance, CurvyClamping clamping, bool fastMode)
        {
            ...
            {
                nextSegInRange = getNextSegmentWithinRange(e.ControlPoint, e.TF, e.Delta * e.Direction, out nextSegInRangeTF); // if dir=-1, this gets the same segment
