...
namespace FluffyUnderware.Curvy
{
    ...
    public class CurvyController : DTVersionedMonoBehaviour
    {
        ...
        {
            if (UpdateIn == CurvyUpdateMethod.Update && Application.isPlaying && Speed!=0)
                Refresh();
        }

        protected virtual void LateUpdate()
        {
            if (UpdateIn == CurvyUpdateMethod.LateUpdate && Speed != 0)
                Refresh();
        }

        protected virtual void FixedUpdate()
        {
            if (UpdateIn == CurvyUpdateMethod.FixedUpdate && Speed != 0)
                Refresh();
        }
